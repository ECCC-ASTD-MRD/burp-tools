...
; $Id: map_2points_new.pro,v 1.2 2008/05/12 13:44:58 afsdhmd Exp $
;
...
Function Map_2points_New, lon0, lat0, lon1, lat1, $
            DPATH=dPath, $
            METERS=meters, $
            MILES=miles, $
            NPATH=nPath, $
...
if ((lat0 eq lat1) and (lon1 eq lon0)) then return,0.
pi2 = !dpi/2
...
if keyword_set(rhumb) then begin ;Rhumb line section
    ...
    lr0 = lat0 * k
    lr1 = lat1 * k
                                ;Check for either point being the pole
    if abs(lr0) ge pi2 then return, [-lr0/k, abs(lr0-lr1)/k]
    if abs(lr1) ge pi2 then return, [lr1/k, abs(lr0-lr1)/k]

    y0 = alog(tan(!dpi/4 + lr0 / 2)) ;Mercator y coordinates
    y1 = alog(tan(!dpi/4 + lr1 / 2))

    Az = atan(x1, y1-y0)
; S is the angular distance between points, in radians.
    if lr0 ne lr1 then s = (lr1-lr0)/cos(Az) else s = abs(x1) * cos(lr0)

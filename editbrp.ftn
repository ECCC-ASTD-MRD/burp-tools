...
*     - DEF7       - DEF1(7)
     ...
     X         BDESIRE, QDFERR, FERMBD, JULM, OUVREBD, EXFIN, NEWSRC,
     X         CMCDATE, READLX, OPDATE, FNOM, OUVREBF, SPOOL, NEWDST,
     X         RESTDEZ, MRFOPC, FERMBF, DATC, BEXCLUR, BARF,  QLXINX,
     X         PREFIXS, PREFIXD

      INTEGER  MOIN1, MOIN2, QDFERR, DATC, FNOM, BRPCOPI, I, J,
     X         MOIN3, MOIN4, OPDATE, JULM, EXDB, MRFOPC, EXFIN
      ...
      DATA VRAI /.TRUE./
      DATA FAUX /.FALSE./
      CHARACTER*2   KLE(MAXCLE)
      CHARACTER*4096 DEF1(MAXCLE), DEF2(MAXCLE)

*     KLE  = CLES DE LA SEQUENCE D'APPEL.
      DATA KLE /'V', 'd.', 'f.', 'N', 'i.', 'l.', 'M', 'VS', 'C',
     X          'R',  'X', 'DS', 125*'s.'/
*     DEF1 = VALEURE DES CLES PAR DEFAUT
      DATA DEF1/'NON',' ',' ','OUI','$IN','$OUT','ERROR','NON','-1',
     X          'NON', 'NON', 'NON', 125*' '/
*     DEF2 = VALEUR DES CLES SI PRESENTES
      DATA DEF2/'OUI',' ',' ','NON','$IN','$OUT','INFORMATIF', 'OUI',
     X          '-1', 'OUI', 'OUI', 'OUI', 125*' '/
...
*     IMPRIME L'INDICATIF DE DEBUT DU PROGRAMME.
      ...
      IF( BOX ) THEN
         I = EXDB('EDITBRP','V3.14.0','NON')
      ELSE
         PRINT*,'***   E D I T B R P   V3.14.0   ***'
      ENDIF
...
*     OUVRE LES FICHIERS SOURCES
   ...
   20    CONTINUE
      IF(NFS .gt. 0) THEN
         ...
         IF(NFS .eq. 0) I = QDFERR('EDITBRP', 'FICHIER SOURCE INCONNU',
     +                              6, 48)
      ENDIF
...
*     LIT UN ENSEMBLE DE DIRECTIVES?
      IF( DEF1(5) .NE. 'NONE' ) THEN
         ...
         CALL READLX(5, I, J)
         IF(I .LT. 0) I = QDFERR
     X      ('EDITBRP', 'ERREUR DANS LES DIRECTIVES', 6, 48) 
      ENDIF
...
*     IMPRIME L'INDICATIF DE FIN DU PGM.
      IF( BOX ) THEN
         I =  EXFIN('EDITBRP','NORMAL','NON')
      ELSE
         PRINT*,'***   E D I T I O N -  T E R M I N E E  ***'
      ENDIF

...
;;              this 1st verssion support only reading files, so READ will
;;              be set anyways 
...
file_unit=long(file_unit)
  ...
  ENDIF ELSE begin
     stind=i_stind 
  ENDELSE
  IF NOT(KEYWORD_SET(i_idtyp)) THEN BEGIN
     idtyp=-1L 
  ENDIF ELSE begin
     idtyp=i_idtyp 
  ENDELSE
  IF NOT(KEYWORD_SET(i_lat)) THEN begin
     lat=-1L 
  ENDIF ELSE begin
     lat=i_lat 
  ENDELSE
  IF NOT(KEYWORD_SET(i_lon)) THEN begin
    lon=-1L
  ENDIF ELSE begin
    lon=i_lon 
  ENDELSE
  IF NOT(KEYWORD_SET(i_date)) THEN begin
    date=-1L
  ENDIF ELSE begin
    date=i_date
  ENDELSE
  IF NOT(KEYWORD_SET(i_heure)) THEN begin
    heure=-1L
  ENDIF ELSE begin
    heure=i_heure
  ENDELSE
  IF NOT(KEYWORD_SET(i_sup)) THEN begin
    sup=-1L
  ENDIF ELSE begin
    sup=i_sup
  ENDELSE
  IF NOT(KEYWORD_SET(i_nsup)) THEN begin
    nsup=0L
  ENDIF ELSE begin
    nsup=i_nsup
  ENDELSE
...
FUNCTION mrbloc,BUF=buf, BLK=i_blk ,BFAM=i_bfam,BDESC=i_bdesc, BTYP=i_btyp,HELP=help, DEBUG=debug
  ...
  IF NOT(KEYWORD_SET(i_blk)) THEN begin
     blk0=0
  ENDIF ELSE begin
     blk0=i_blk
  ENDELSE
  IF NOT(KEYWORD_SET(i_bfam)) THEN begin
     bfam=-1L
  ENDIF ELSE begin
     bfam=i_bfam
  ENDELSE
  IF NOT(KEYWORD_SET(i_btyp)) THEN BEGIN
     btyp=-1L
  ENDIF ELSE begin
     btyp=i_btyp
  ENDELSE
  IF NOT(KEYWORD_SET(i_bdesc)) THEN begin
     bdesc=-1L
  ENDIF ELSE begin
     bdesc=i_bdesc
  ENDELSE
...
FUNCTION blk_type_marqueur,BTYPE=i_btype, HELP=help 
  ...
  IF NOT(KEYWORD_SET(i_btype)) THEN begin
     btype=0L
  ENDIF ELSE begin
     btype=long(i_btype)
  ENDELSE
...
print,"| istat = BLK_TYPE_MARQUEUR (..) returns 1  blok marquor, 0 othewise     |"
print,"|                                                                        |"

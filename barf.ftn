...
*                  LA ROUTINE MEMOIR RESERVE L'ESPASSE.
     ...
     X             DX, DY, J, MRFPRM, MRFGET, LONG, TROUVE, IREC, EXTRA,
     X             FG, LG, K, MRFPUT, BUFSIZ, AJOU, MRFOPC, DATE, IP(8),
     X             HD, DT, L, IST, MD   
      EQUIVALENCE  (IP(1), LT), (IP(2), LN), (IP(4), DT), (IP(6), DX),
     X             (IP(7), DY), (IP(5), FG), (IP(8), LG)
      CHARACTER *3 TIP(3)
      CHARACTER *9 STN, WILD
...
*        Y A-T-IL UN AUTRE ENREGISTREMENT.
   20    IREC = MRFLOC(SOURCES(L), IREC, WILD, -1, -1, -1, -1, -1, I, 0)
         IF(IREC .LT. 0) THEN
            PRINT*,'FIN DU FICHIER',SOURCES(L)
            GO TO 130
         ENDIF
         IF( MODINF ) THEN
            I = MRFPRM(IREC, STNID, DT, LT, LN, DX, DY, DATE, TEMP, FG,
     X                 EXTRA, 0, LG)
            IP(3) = JULM(DATE, TEMP)
            IF(LN .EQ. 3600) LN = 0  
         ENDIF
...
*              ON A UNE LISTE DE STNID A TROUVER 
               DO 50 J=1,STNS(K)
                  ...
                  DO 40 I=1,9
                    IF(STN(I:I).NE.'*' .AND.
     X                  STN(I:I).NE.STNID(I:I)) GO TO 50
   40                CONTINUE
                  GO TO 60
   50             CONTINUE
               GO TO 110
...
*           VALIDE  LAT, LON, DATE, TYPE, FLAG, DX, DY, LONG
   60       BOITE = (REQ(11,1,K).EQ.-1) .AND. (REQ(3,1,K).EQ.1) .AND.
     X              (REQ(11,2,K).EQ.-1) .AND. (REQ(3,2,K).EQ.1)  
            DO 90 J=1,NARG(K)
...
*                    L'ARGUMENT EST UN INTERVALE AVEC SAUT
                     IF((IP(J) .LT. REQ(1,J,K)) .OR.
     X                  (IP(J) .GT. REQ(2,J,K))) GO TO 110
*                    BOITES DE LAT, LON AVEC STATION AU POLE
                     ...
                     ELSE
                        IF((MOD((IP(J)-REQ(1,J,K)),REQ(3,J,K)).EQ.0))
     X                  GOTO 90
                     ENDIF
                     GO TO 110
                  ENDIF
...
*              REQUETE = DESIRE, COPIER SI PAS EN MODE STATISTIQUE
     ...
     X                       EXTRA, 0)
               ...
               RENDUA = RENDUA - 1
               IF( ECHO ) WRITE(6,600) TIP(MD), STNID, LT, LN, DX,
     X                            DY, FG, DATE, TEMP, DT, LG, IREC
               IF(RENDUA .EQ. 0) THEN
                  IF( INFORM ) PRINT*,'MAXIMUM DE COPIES ATTEINT'
                  GO TO 140
               ENDIF
            ELSE
*              DIRECTIVE SATISFAITE  = EXCLURE
               EXCLUS = EXCLUS + 1
               IF( ECHO ) WRITE(6,600) TIP(1), STNID, LT, LN, DX, DY,
     X                                 FG, DATE, TEMP, DT, LG, IREC
            ENDIF
            GO TO 20
  110       CONTINUE
         GO TO 20
*        AUCUNE DES NREQ DESIRE/EXCLURE SATISFAITE
  ...
  140 I = MRFOPC('MSGLVL', DEF7)
      ...
      DO 150 I=1,NREQ
         PRINT*,'REQUETE',I,' SATISFAITE',SATISF(I),' FOIS'
  150    CONTINUE
      IF( STATS ) THEN
         PRINT*,'ENREGISTREMENTS TROUVES =', TROUVE
      ELSE
         IF( REECR  .GT.0) PRINT*,'ENREGISTREMENTS REECRITS =', REECR
         IF( AJOU   .GT.0) PRINT*,'ENREGISTREMENTS AJOUTES  =', AJOU
         IF( EXCLUS .GT.0) PRINT*,'ENREGISTREMENTS EXCLUS   =', EXCLUS
      ENDIF
      EXPRESS = .FALSE.
      RETURN
 ...
 601  FORMAT(' MODE STATION     LATI   LONG     DX     DY  FLGS(HEX) ',
     x       '   DATE  TEMPS  IDTYP  LONGUEUR   ADRESSE')

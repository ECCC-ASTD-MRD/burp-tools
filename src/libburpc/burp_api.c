...
void brp_allocrpt( BURP_RPT *rpt, int  nsize )
{
   ...
   if ( rpt == NULL )
   {
      Lib_Log(APP_MAIN,APP_ERROR,"%s: rpt pointer is NULL\n",__func__);
      return ;
   }
...
void brp_allocblk( BURP_BLK *blk, int  nele, int nval, int nt )
{
        ...
        if (nele * nval * nt == 0 )
        {
            Lib_Log(APP_MAIN,APP_ERROR,"%s: une des dimensions du block est nulle!!\n",__func__);
            return;
        }
/*
...
int  brp_readblk(int bkno, BURP_BLK  *blk, BURP_RPT  *rpt, int docvt)
{
        ...
        if ((rpt == NULL)) {
           Lib_Log(APP_MAIN,APP_ERROR,"%s: attention rpt nul \n",__func__);
           return -1;
        }
        if ((blk == NULL)) {
           Lib_Log(APP_MAIN,APP_ERROR,"%s: attention blk nul \n",__func__);
           return -1;
        }
/*
** lire les informations du bloc
...
BURP_RPT *brp_newrpt( void )
{
     ...
     if (rpt == NULL) Lib_Log(APP_MAIN,APP_ERROR,"%s: Out of memory!\n",__func__);
...
void brp_copyblk( BURP_BLK *dest, const BURP_BLK *source)
{
    ...
    if (source == NULL)
    {
      Lib_Log(APP_MAIN,APP_ERROR,"%s: blk pointer is NULL, brp_copyblk not done!\n",__func__);
      return;
    }
...
void brp_resizeblk( BURP_BLK *dest,int nele, int nval, int nt)
{
    ...
    if (dest == NULL)
    {
        Lib_Log(APP_MAIN,APP_ERROR,"%s: blk pointer is NULL, resizeblk not done!\n",__func__);
        return;
    }
    source = brp_newblk();
...
int brp_delblk(BURP_RPT *rpt, const BURP_BLK *blk)
{
   ...
   {
      Lib_Log(APP_MAIN,APP_ERROR,"%s: rpt adn blk pointers are NULL\n",__func__);
      return istat;
   }
   if (rpt->buffer != NULL)
...
void brp_copyrpthdr( BURP_RPT * dest, const BURP_RPT *source )
{
   if (source == NULL)
   {
      Lib_Log(APP_MAIN,APP_ERROR,"%s: rpt source  pointer is NULL\n",__func__);
      return ;
   }
 ...
 int brp_open(int  iun, const char *filename, char *op)
{
   ...
   if ( istat != 0 )
   {
       Lib_Log(APP_MAIN,APP_ERROR,"%s: Unable to open file as %s : %s\n",__func__,type,filename);
       c_fclos( iun );
       return istat;
   }

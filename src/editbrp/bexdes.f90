...
!                 LA FENETRE SE FINIT A [@,AAMMJJ,HHMM]
                  ...
                  write(app_msg,*) 'bexdes: The window of ',W,' minutes(s)',' ends at ',REQ(2,M,NREQ)
...
!                 LA FENETRE SE DEBUTE A [AAMMJJ,HHMM,@]
                  ...
                  write(app_msg,*) 'bexdes: The window of ',W,' minutes(s)',' begins at ',REQ(1,M,NREQ)
                  call App_Log(APP_INFO,app_msg)
               ENDIF
            ELSE
!              [AAMMJJ,HHMM]  = LE CENTRE DE LA FENETRE
               ...
               REQ(2,M,NREQ)  = REQ(1,M,NREQ) + W - 1
               write(app_msg,*) 'bexdes: The window of ',W,' minutes(s)',' centers at ',I
               call App_Log(APP_INFO,app_msg)
         ENDIF  
         ELSEIF( SAUT ) THEN
...
!              CHANGE REQUETE DE TYPE  [@,AAMMJJ,HHMM,DELTA,JJHHMM]
               ...
               ENDIF
               REQ(2,M,NREQ) = JULM(CLE(2), J)
               write(app_msg,*) 'bexdes: The period ends at',CLE(2),J,' DELTA=',REQ(3,M,NREQ),' minute(s)'
               call App_Log(APP_INFO,app_msg)
         ELSE
...
!                 FENETRE DEBUTANT LE AAMMJJ HHMM
                  write(app_msg,*) 'bexdes: The period begins at',(CLE(I),I=1,2),' DELTA=',REQ(3,M,NREQ),' MINUTE(S)'
                  call App_Log(APP_INFO,app_msg)
                  ELSE
                  IF(N .EQ. 4) THEN
...
!                 REQUETE TYPE  [AAMMJJ,HHMM,@,AAMMJJ,HHMM,DELTA,JJHHMM]
                  ...
                  write(app_msg,*) 'bexdes: The period begins at',(CLE(I),I=1,2),' ends at', &
                     CLE(3),J,' DELTA=',REQ(3,M,NREQ),' minute(s)'
                  call App_Log(APP_INFO,app_msg)
                  ENDIF
            ENDIF
...
!           PRENDRE LA DATE DE LA PERIODE?
            IF(JOURS(4) .EQ. 0) THEN
               call App_Log(APP_WARNING,'bexdes: No valid range of period directive')
            ELSE
               write(app_msg,*) 'bexdes: DATE = date or common period'
               call App_Log(APP_INFO,app_msg)
...
!           LISTE DE 1 A 5 DATE BURP
   ...
   20             REQ(K,M,NREQ) = JULM(CLE(I), CLE(I+1))
            ENDIF
            REQ(11,M,NREQ) = K
            write(app_msg,600) (CLE(J),J=1,N)
            call App_Log(APP_INFO,app_msg)
  600       FORMAT('bexdes: DATE(S)=',5(I6,',',I4,3X))
         ENDIF
      ELSE

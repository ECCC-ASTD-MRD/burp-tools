...
*
      SUBROUTINE BDESIRE(STATION, LAT, LON, DAT, TYPE, FLAG, DTX, DTY,
     X                   LENG)
                   
      IMPLICIT NONE 
  
      INTEGER  TYPE(10), LAT(10), LON(10), DAT(20), FLAG(10),
     X         LENG(10), DTX(10), DTY(10), STATION(30)    
*
*AUTEUR
...
#include "char.cdk"
      ...
      EXTERNAL ARGDIMS, ARGDOPE, QDFERR, BEXDES
      INTEGER  ARGDIMS, ARGDOPE, QDFERR, LISTE(10), I, J, K, DEX,
     X         NC, PO
      DEX = -1
    1 IF(NREQ .EQ. NMD) THEN
         PRINT*,'*** MAXIMUM DE REQUETES DESIR DEJA ATTEINT ***'
         RETURN
      ENDIF
  
*     SI DEJA EN MODE EXPRESS CDETTE DIRECTIVE EXT INUTILE
      IF( EXPRESS ) THEN
         PRINT*,'*** DIRECTIVE INUTILE ***'
         RETURN
      ENDIF
...
*     COMPTE LES DIRECTIVES 
      NREQ = NREQ+1 
      NARG(NREQ) = NP-1
      IF( INFORM ) WRITE(6,*) 'REQUETE #',NREQ
...
*     TRIAGE PAR LENG
   ...
   30       CONTINUE
         IF( INFORM ) WRITE(6,*)'LENG= ',(REQ(I,8,NREQ),I=1,ARGDIMS(9))
      ENDIF
...
*     TRIAGE PAR DTY
   40 IF(DTY(1) .eq. -1) THEN
      ...
      ELSE
         ...
         IF( INFORM ) WRITE(6,*)'DTY  = ',(REQ(I,7,NREQ),I=1,ARGDIMS(8))
      ENDIF
...
*     TRIAGE PAR DTX
   50 IF(DTX(1) .eq. -1) THEN
      ...
      ELSE
         ...
         IF( INFORM ) WRITE(6,*)'DTX  = ',(REQ(I,6,NREQ),I=1,ARGDIMS(7))
      ENDIF
   ...
   60 IF(FLAG(1) .eq. -1) THEN
      ...
      ELSE
         ...
         DO 70 J=1,ARGDIMS(6)
            REQ(J,5,NREQ) = FLAG(J)
   70       CONTINUE
         IF( INFORM ) WRITE(6,*)'FLAGS= ',(REQ(I,5,NREQ),I=1,ARGDIMS(6))
      ENDIF
...
*     TRIAGE PAR TYPE
   80 IF(TYPE(1) .eq. -1) THEN
      ...
      ELSE
         ...
         IF( INFORM ) WRITE(6,*)'TYPE= ',(REQ(I,4,NREQ),I=1,ARGDIMS(5))
      ENDIF
...
*     TRIAGE PAR LON
  100 IF(LON(1) .eq. -1) THEN
      ...
      ELSE
         ...
         IF( INFORM ) WRITE(6,*)'LON = ',(REQ(I,2,NREQ),I=1,ARGDIMS(3))
      ENDIF
...
*     TRIAGE PAR LAT 
  110 IF(LAT(1) .eq. -1) THEN
      ...
      ELSE
         ...
         IF( INFORM ) WRITE(6,*)'LAT = ',(REQ(I,1,NREQ),I=1,ARGDIMS(2))
      ENDIF
...
*     TRIAGE PAR STNID
  120 IF(STATION(1) .NE. -1) THEN
         ...
         DO 130 K=1, STNS(NREQ)
            ...
            IF(NC .GT. 9) I = QDFERR
     X      ('bdesire', 'STNID LIMITE A 9 CARACTERES', 6, 48)
            IF(IAND(255, LISTE(K)) .NE. 3) I = QDFERR
     X      ('bdesire', 'STNID MAL PASSE', 6, 48)
            I  = NC / NCW
            IF(I*NCW .LT. NC) I = I+1  
            WRITE(ID(K,NREQ), CAR9) (STATION(J),J=PO,PO+I-1)
  130       CONTINUE
         IF( INFORM ) WRITE(6,*)'STNID=', (ID(I,NREQ),I=1,STNS(NREQ))
      ELSEIF(NP .eq. 1) THEN
         IF( DEX.eq.0 .and. NREQ.eq.NEXC )
     X      I = QDFERR('bdesire', 'EXCLURE TOUT LE FICHIER?', 6, 48) 

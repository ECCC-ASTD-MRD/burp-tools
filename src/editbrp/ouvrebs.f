...
#include "fiches.cdk"
**
      EXTERNAL FERMBD, MRFOPN, MRFNBR, QDFERR, FCLOS, NUMBLKS,
     X         FERMBF, FERMBS, MRFCLS, MRFVOI, FNOM, MRFAPP
      INTEGER  MRFOPN, MRFNBR, MRFCLS, QDFERR, FNOM, NUMBLKS,
     X         MRFVOI, FCLOS,  I, J, K, MRFAPP

*     FERMER LES FICHIERS DEJA OUVERTS?  
      IF(NFO .GT. 0) THEN
*        SI "LE" FICHIER A OUVRIR DEJA OUVERT 
         IF( (NFO*NFS.eq.1) .and. (DN(1).eq.NS(1)) ) THEN
            IF( INFORM ) PRINT*,'FICHIER DEJA OUVERT'
            RETURN
         ELSE
            CALL FERMBS
         ENDIF   
     ...
     X    'RND+OLD',0) .ne. 0) THEN
         ...
         ELSE
            IF( NUMBLKS(SOURCES(NFO+1)) .EQ. 0) THEN
               J = FCLOS( SOURCES(NFO+1) )
               IF( INFORM ) PRINT*,'LE FICHIER ',DN(K),'VIDE'
            ELSE
               NFO     = NFO + 1
               NS(NFO) = DN(K)
...
*     EN MODE STATS SI FICHIER DESTINATION PAS DONNE
      ...
      STATS = ND .EQ. ' ' 
      IF( STATS ) THEN
         IF(INFORM) PRINT*,'EDITBRP EST EN MODE STATS PAS DE COPIE' 
      ELSE
*        SI FICHIER DES FUSUONS = DESTINATION
         IF(ND.EQ.NF .AND. OUVF) CALL FERMBF
*        SI SOURCE = DESTINATION ABORT
         DO 40 I=1,NFO
            IF(ND .EQ. NS(I)) K = QDFERR
     X      ('ouvrebs', 'DESTINATION=SOURCE', 6, 48)
   40       CONTINUE
...
*     SI UN DES SOURCES = FUSION ABORT
      DO 50 I=1,NFS
         IF(NF .EQ. NS(I)) K = QDFERR
     X      ('ouvrebs', 'SOURCE = FICHIER FUSION', 6, 48)
   50    CONTINUE

*     SI FUSION = DESTINATION 
      IF(NF.EQ.ND .AND.OUVD) K = QDFERR
     X      ('ouvrebs', 'DESTINATION = FICHIERFUSION', 6, 48)
